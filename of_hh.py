...
def show_gesamt(flow, year):
  info = query_db('select sum(euro) as main_value, jahr, E_A from haushalt join fachdienste on haushalt.produktbereich = fachdienste.produktbereich where haushalt.jahr = ? and haushalt.E_A = "A"', [year], one=True)
